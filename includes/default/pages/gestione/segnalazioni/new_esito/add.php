...
if ( $_POST['op'] == 'add' ) {
    ...
    <?php } else {
        if ( isset($_POST['note']) === false ) {
            $note = 'Nessuna';
        } else {
            $note = gdrcd_filter('in', $_POST['note']);
        }
        if ( isset($_POST['chat']) === false ) {
            $chat = 0;
        } else {
            $chat = gdrcd_filter('num', $_POST['chat']);
        }
        if ( isset($_POST['id_ab']) === false ) {
            $ab = 0;
        } else {
            $ab = gdrcd_filter('num', $_POST['id_ab']);
        }
        if ( isset($_POST['dice_face']) === false ) {
            $facce = 0;
        } else {
            $facce = gdrcd_filter('num', $_POST['dice_face']);
        }
        if ( isset($_POST['dice_num']) === false ) {
            $num = 0;
        } else {
            $num = gdrcd_filter('num', $_POST['dice_num']);
        }
        if ( isset($_POST['mod']) === false ) {
            $mod = 0;
        } else {
            $mod = gdrcd_filter('num', $_POST['mod']);
        }
