...
$pagebegin = $PARAMETERS['settings']['records_per_page_calendar'] * gdrcd_filter('num', $_REQUEST['offset']);

//Conteggio record totali
$today = date("Y-m-d");

$record_globale = gdrcd_query("SELECT COUNT(*) as tot FROM eventi WHERE start > '{$today}'");
...
$query = "SELECT eventi.id, titolo, descrizione , start, end,eventi_tipo.title, eventi_colori.backgroundColor, eventi_colori.textColor, eventi_colori.colore  FROM eventi 
                ...
                WHERE start > '{$today}' order by start ASC LIMIT {$pageend} OFFSET {$pagebegin}";
...
while ($row = gdrcd_query($result, 'fetch')) {
    ...
    <div class="intestazione_eventi">
        <?= gdrcd_bbcoder(gdrcd_filter('out', $row['title'])); ?> - Data
        inizio: <?= gdrcd_filter('out', $row['start']) ?>;
