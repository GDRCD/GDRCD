<div class="pagina_gestione_tipi">
    ...
    { ?>
        ...
        <!-- Titolo della pagina -->
        <?php if (isset($_REQUEST['types']) === false)
    {
        $types = 'items';
    } else
    {
        $types = gdrcd_filter('get', $_REQUEST['types']);
    } ?>

        <div class="page_title">
            <h2><?php echo gdrcd_filter('out',
                    $MESSAGE['interface']['administration']['types']['page_name'][$types]); ?></h2>
        </div>
        ...
        <div class="page_body">
            ...
            if ($_POST['op'] == 'insert')
            {
                ...
                if ($types == "items")
                {
                    $query = "INSERT INTO codtipooggetto (descrizione) VALUES ('" . gdrcd_filter('in',
                            $_POST['nome']) . "')";
                }
                if ($types == "guilds")
                {
                    $query = "INSERT INTO codtipogilda (descrizione) VALUES ('" . gdrcd_filter('in',
                            $_POST['nome']) . "')";
                }
                gdrcd_query($query); ?>
                <div class="warning">
                ...
                <div class="link_back">
                    <a href="main.php?page=gestione_tipi&types=<?php echo $types; ?>">
                        <?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['administration']['types']['link']['back']); ?>
                    </a>
                </div>
            <?php } ?>
            ...
            if ($_POST['op'] == 'erase')
            {
                ...
                if ($types == "items")
                {
                    $query = "DELETE FROM codtipooggetto WHERE cod_tipo=" . gdrcd_filter('num',
                            $_POST['id_record']) . " LIMIT 1";
                    $query_update = "UPDATE oggetto SET tipo = 0 WHERE tipo = " . gdrcd_filter('num',
                            $_POST['id_record']) . "";
                }
                if ($types == "guilds")
                {
                    $query = "DELETE FROM codtipogilda WHERE cod_tipo=" . gdrcd_filter('num',
                            $_POST['id_record']) . " LIMIT 1";
                    $query_update = "UPDATE gilda SET tipo = 0 WHERE tipo = " . gdrcd_filter('num',
                            $_POST['id_record']) . "";
                }
                gdrcd_query($query);
            ...
            if ($_POST['op'] == 'modify')
            {
                ...
                if ($types == "items")
                {
                    $query = "UPDATE codtipooggetto SET descrizione ='" . gdrcd_filter('in',
                            $_POST['nome']) . "' WHERE cod_tipo = " . gdrcd_filter('num',
                            $_POST['id_record']) . " LIMIT 1";
                }
                if ($types == "guilds")
                {
                    $query = "UPDATE codtipogilda SET descrizione ='" . gdrcd_filter('in',
                            $_POST['nome']) . "' WHERE cod_tipo = " . gdrcd_filter('num',
                            $_POST['id_record']) . " LIMIT 1";
                }
                gdrcd_query($query); ?>
            ...
            if ((gdrcd_filter('get', $_POST['op']) == 'edit') || (gdrcd_filter('get', $_REQUEST['op']) == 'new'))
            {
                ...
                if (gdrcd_filter('get', $_POST['op']) == 'edit')
                {
                    ...
                    if ($types == "items")
                    {
                        $query = "SELECT * FROM codtipooggetto WHERE cod_tipo=" . gdrcd_filter('in',
                                $_POST['id_record']) . " LIMIT 1 ";
                    }
                    if ($types == "guilds")
                    {
                        $query = "SELECT * FROM codtipogilda WHERE cod_tipo=" . gdrcd_filter('in',
                                $_POST['id_record']) . " LIMIT 1 ";
                    }
                    ...
                    <form action="main.php?page=gestione_tipi&types=<?php echo $types; ?>"
                        ...
                        <div class='form_submit'>
                            ...
                            if ($operation == "edit")
                            {
                                ...
                                <input type="submit"
                                       value="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['administration']['types']['submit']['edit']); ?>"/>
                            ...
                            { ?>
                                <input type="submit"
                                       value="<?php echo gdrcd_filter('out',
                                           $MESSAGE['interface']['administration']['types']['submit']['insert']); ?>"/>
            ...
            <?php if ((isset($_POST['op']) === false) && (isset($_REQUEST['op']) === false))
            {
                            ...
                            <tr>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['administration']['types']['name']); ?></div>
                                </td>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['administration']['types']['code']); ?></div>
                                </td>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                ...
                <div class="link_back">
                    <a href="main.php?page=gestione_tipi&types=<?php echo $types; ?>&op=new">
                        <?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['administration']['types']['link']['new']); ?>
                    </a>
                </div>
                <?php if ($types == 'guilds')
            { ?>
                <div class="link_back">
                    <a href="main.php?page=gestione_gilde">
                        <?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['administration']['types']['link']['guilds']); ?>
                    </a>
                </div>
            <?php } ?>
                <?php if ($types == 'items')
            { ?>
                <div class="link_back">
                    <a href="main.php?page=gestione_mercato">
                        <?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['administration']['types']['link']['items']); ?>
                    </a>
                </div>
            <?php } ?>
            <?php }//else
