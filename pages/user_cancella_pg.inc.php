    ...
    <div class="page_body">
        ...
        if ($_POST['op'] == 'delete')
        {
            if (($email == gdrcd_filter_email($_POST['email'])) && (gdrcd_check_pass($_POST['new_pass']) === true))
            {
                ...
                gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento, descrizione_evento) VALUES ('" . $_SESSION['login'] . "','" . $_SESSION['login'] . "', NOW(), " . DELETEPG . " ,'" . gdrcd_filter('in',
                        $MESSAGE['interface']['user']['delete']['undeleted']) . "')");
        ...
        {
            gdrcd_query("UPDATE personaggio SET permessi = -1 WHERE nome = '" . gdrcd_filter('in',
            ...
            gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento, descrizione_evento) VALUES ('" . gdrcd_filter('in',
                    $_POST['account']) . "','" . $_SESSION['login'] . "', NOW(), " . DELETEPG . " ,'" . gdrcd_filter('in',
                    $MESSAGE['interface']['user']['delete']['deleted']) . "')");
            ?>
            <div class="warning">
                <?php echo gdrcd_filter('out', $MESSAGE['warning']['modified']); ?>
            </div>
        ...
        if ((gdrcd_filter('get', $_POST['op']) == 'force') && ($_SESSION['permessi'] == SUPERUSER))
        {
            ...
            gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento, descrizione_evento) VALUES ('" . gdrcd_filter('in',
                    $_POST['account']) . "','" . $_SESSION['login'] . "', NOW(), " . DELETEPG . " ,'->')");
        ...
        {
            gdrcd_query("UPDATE personaggio SET permessi = 0 WHERE nome = '" . gdrcd_filter('in',
                    $_POST['account']) . "'");
                        ...
                        <div class="form_field">
                            <select name="account">
                                <option disabled selected><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['user']['delete']['who']); ?></option>
                ...
                <div class="form_gioco">
                    <form action="main.php?page=user_cancella_pg" method="post">
                        ...
                        <div class="form_field">
                            <select name="account">
                                <option disabled selected><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['user']['get_back']['who']); ?></option>
                                <?php while ($row = gdrcd_query($result, 'fetch'))
                                { ?>
                                    <option value="<?php echo $row['nome']; ?>"><?php echo $row['nome']; ?></option>
                                <?php }//while
