    ...
    { ?>
        ...
        <div class="page_title">
            <h2><?php echo gdrcd_filter('out',
                    $MESSAGE['interface']['administration']['log']['messages']['page_name']); ?></h2>
        </div>
        ...
        <div class="page_body">
                ...
                <div class="panels_box">
                    <div class="form_gestione">
                        <form action="main.php?page=log_messaggi"
                              method="post">
                            <?php
                            $result = gdrcd_query("SELECT nome FROM personaggio WHERE permessi > " . DELETED . " ORDER BY nome",
                                'result'); ?>
                            <div class='form_label'>
                                <?php echo gdrcd_filter('out',
                                    $MESSAGE['interface']['administration']['log']['messages']['log_type']); ?>
                            </div>
            ...
            if (isset($_REQUEST['op']) == 'view')
            {
                ...
                $record_globale = gdrcd_query("SELECT COUNT(*) FROM backmessaggi WHERE mittente = '" . gdrcd_filter('in',
                        $_REQUEST['pg']) . "'");
                $totaleresults = $record_globale['COUNT(*)'];
                //Lettura record
                $result = gdrcd_query("SELECT destinatario, spedito, testo FROM backmessaggi WHERE mittente = '" . gdrcd_filter('in',
                        $_REQUEST['pg']) . "' ORDER BY spedito DESC LIMIT " . $pagebegin . ", " . $pageend . "",
                    'result');
                            ...
                            <tr>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco">
                                        <?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['administration']['log']['messages']['dest']); ?>
                                    </div>
                                </td>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco">
                                        <?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['administration']['log']['messages']['date']); ?>
                                    </div>
                                </td>
                                <td class="casella_titolo">
                                    <div class="titoli_elenco">
                                        <?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['administration']['log']['messages']['text']); ?>
                                    </div>
                                </td>
                            </tr>
                            <!-- Record -->
