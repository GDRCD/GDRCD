...
<div class="pagina_servizi_banca">
    ...
    <div class="page_body">
        ...
        if ((isset($_POST['op']) === true) && (gdrcd_filter('get', $_POST['op']) == 'preleva'))
        {
            ...
            } else
            {
                if ($_POST['ammontare'] > $banca)
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['bank']['withdraw_no']) . '</div>';
                } else
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['bank']['done']) . '</div>';
            ...
            <div class="link_back">
                <a href="main.php?page=servizi_banca"><?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['bank']['back']); ?></a>
            </div>
        <?php } ?>
        ...
        if ((isset($_POST['op']) === true) && (gdrcd_filter('get', $_POST['op']) == 'deposita'))
        {
            ...
            } else
            {
                ...
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['bank']['deposit_no']) . '</div>';
                } else
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['bank']['done']) . '</div>';
                    ...
                    gdrcd_query("UPDATE personaggio SET soldi = soldi - " . gdrcd_filter('num',
                            $_POST['ammontare']) . ", banca = banca + " . $_POST['ammontare'] . " WHERE nome = '" . $_SESSION['login'] . "' LIMIT 1");
                }
            } ?>
        ...
        if ((isset($_POST['op']) === true) && ($_POST['op'] == 'bonifico'))
        {
            ...
            } else
            {
                ...
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['bank']['error']) . '</div>';
                } else
                {
                    if ($_POST['ammontare'] > $banca)
                    {
                    ...
                    } else
                    {
                        echo '<div class="warning">' . gdrcd_filter('out',
                                $MESSAGE['interface']['bank']['done']) . '</div>';
                        /*Eseguo la transazione*/
                        gdrcd_query("UPDATE personaggio SET banca = banca - " . gdrcd_filter('num',
                                $_POST['ammontare']) . " WHERE nome = '" . $_SESSION['login'] . "' LIMIT 1");
                        gdrcd_query("UPDATE personaggio SET banca = banca + " . gdrcd_filter('num',
                                $_POST['ammontare']) . " WHERE nome = '" . $_POST['beneficiario'] . "' LIMIT 1");
                        ...
                        gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento ,descrizione_evento) VALUES ('" . gdrcd_filter('in',
                                $_POST['beneficiario']) . "', '" . $_SESSION['login'] . "', NOW(), " . BONIFICO . ", '" . '(' . gdrcd_filter('num',
                                $_POST['ammontare']) . ' ' . $PARAMETERS['names']['currency']['plur'] . ') ' . gdrcd_filter('in',
                                $_POST['causale']) . "')");
                        gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "','" . gdrcd_capital_letter(gdrcd_filter('in',
                                $_POST['beneficiario'])) . "', NOW(), '" . gdrcd_filter('in',
                                $_SESSION['login'] . ' ' . $MESSAGE['interface']['bank']['notice'] . ' ' . gdrcd_filter('num',
                                    $_POST['ammontare']) . ' ' . $PARAMETERS['names']['currency']['plur']) . '. \n\n' . gdrcd_filter('in',
                                $_POST['causale']) . "')");
                    }
                }
            } ?>
            ...
            <div class="panels_box">
                <div class="status_bancario">
                    ...
                    <?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['bank']['amount'] . ": " . $banca . " " . $PARAMETERS['names']['currency']['plur']); ?>
                    <br/>
                    <!-- Stipendio -->
                    <?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['bank']['per_day'] . ": " . $stipendio . " " . $PARAMETERS['names']['currency']['plur']); ?>
                    <br/>
                    <!-- In tasca -->
                    <?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['bank']['pocket'] . ": " . $soldi . " " . $PARAMETERS['names']['currency']['plur']); ?>
                    <br/>
                </div>
                            ...
                            <select name="beneficiario" class="form_gestione_selectbox">
                                ...
                                { ?>
                                    <option value="<?php echo $option['nome']; ?>">
                                        <?php echo gdrcd_filter('out', $option['nome']) . " " . gdrcd_filter('out',
                                                $option['cognome']); ?>
                                    </option>
                                <?php }//while
                        ...
                        <div class='form_submit'>
                            <input name="conferma"
                                   ...
                                   value="<?php echo gdrcd_filter('out',
                                       $MESSAGE['interface']['bank']['execute']); ?>"/>
                        </div>
                    </form>
                </div>
                    ...
                    { ?>
                        <div class="form_gioco">
                            <form
                                ...
                                <div class="form_label">
                                    <?php echo gdrcd_filter('out',
                                            $MESSAGE['interface']['bank']['pay']) . ' (' . gdrcd_filter('out',
                                            $MESSAGE['interface']['bank']['credit']) . ': ' . $stipendio . ' ' . $PARAMETERS['names']['currency']['plur'] . ') '; ?>
                                </div>
                                <div class='form_submit'>
