...
<div class="pagina_forum">
    ...
    <div class="page_body">
        ...
        if ($_POST['op'] == 'insert')
        {
            ...
            $thread = gdrcd_query("SELECT araldo.id_araldo, araldo.tipo, araldo.proprietari" . $fields . " FROM araldo " . $join . (! empty($cond) ? ' WHERE ' . $cond : ''),
                'result');
            ...
            if (gdrcd_query($thread, 'num_rows'))
            {
                ...
                if (($araldoData['tipo'] == SOLORAZZA and ($_SESSION['id_razza'] == $araldoData['proprietari'] || $_SESSION['permessi'] >= MODERATOR)) ||
                    ($araldoData['tipo'] == SOLOGILDA and (strpos($_SESSION['gilda'],
                                '*' . $araldoData['proprietari'] . '*') !== false || $_SESSION['permessi'] >= MODERATOR)) ||
                ...
                )
                {
                    ...
                    gdrcd_query("INSERT INTO messaggioaraldo (id_messaggio_padre, id_araldo, titolo, messaggio, autore, data_messaggio ) VALUES (" . gdrcd_filter('num',
                            $_POST['padre']) . ", " . gdrcd_filter('num',
                            $araldoData['id_araldo']) . ", '" . gdrcd_filter('in',
                            $_POST['titolo']) . "', '" . gdrcd_filter('in',
                            $_POST['messaggio']) . "', '" . gdrcd_filter('in', $_SESSION['login']) . "', NOW())");
                    ...
                    <?php
                    gdrcd_query("DELETE FROM araldo_letto WHERE thread_id = " . gdrcd_filter('num',
                            $_POST['padre']) . " AND nome != '" . $_SESSION['login'] . "'");

                    gdrcd_redirect('main.php?page=forum&op=read&what=' . gdrcd_filter('num',
                            $_POST['padre']) . '&where=' . $araldoData['id_araldo']);
                } else
                {
            ...
            } else
            {
                echo '<div class="warning">' . gdrcd_filter('out',
                        $MESSAGE['interface']['administration']['forums']['not_exists']) . '</div>';
            }
        } //Fine Insert
        ...
        if ($_POST['op'] == 'edit')
        {
            $row = gdrcd_query("SELECT autore, titolo, messaggio, id_messaggio_padre FROM messaggioaraldo WHERE id_messaggio=" . gdrcd_filter('num',
                    $_POST['id_messaggio']) . "");
            ...
            if ($row['autore'] == $_SESSION['login'] || ($row['autore'] != $_SESSION['login'] && $_SESSION['permessi'] >= MODERATOR))
            {
                ...
                gdrcd_query("UPDATE messaggioaraldo SET messaggio = '" . gdrcd_filter('in',
                        $_POST['messaggio']) . '\n\n\n\nEdit (' . $_SESSION['login'] . '): ' . $time . "', titolo = '" . gdrcd_filter('in',
                        $_POST['titolo']) . "' WHERE id_messaggio = " . gdrcd_filter('num',
                        $_POST['id_messaggio']) . " LIMIT 1");
                ?>
                <div class="warning">
                ...
                {
                    gdrcd_redirect('main.php?page=forum&op=read&what=' . gdrcd_filter('num',
                            $_POST['id_messaggio']) . '&where=' . gdrcd_filter('num', $_POST['araldo']));
                } else
                {
                    gdrcd_redirect('main.php?page=forum&op=read&what=' . gdrcd_filter('num',
                            $row['id_messaggio_padre']) . '&where=' . gdrcd_filter('num', $_POST['araldo']));
                }
            } else
            {
                ?>
        ...
        {
            $row = gdrcd_query("SELECT titolo, messaggio, id_messaggio_padre FROM messaggioaraldo WHERE id_messaggio=" . gdrcd_filter('num',
                    $_REQUEST['what']) . "");
            ?>
                            ...
                            if ($padre == -1)
                            {
                                ...
                                <div class="form_label">
                                    <?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['forums']['insert']['title']); ?>
                                </div>
                                <div class="form_field">
                                    <input name="titolo"/>
                                </div>
...
?>
        ...
        if ($_REQUEST['op'] == 'read')
        {
            ...
            if ( ! empty($row))
            {
                ...
                if ((($row['tipo'] == SOLORAZZA) && ($_SESSION['id_razza'] != $row['proprietari']) && ($_SESSION['permessi'] < MODERATOR)) ||
                    (($row['tipo'] == SOLOGILDA) && (strpos($_SESSION['gilda'],
                                '*' . $row['proprietari'] . '*') === false) && ($_SESSION['permessi'] < MODERATOR)) ||
                ...
                } else
                {
                    ...
                    $check_letto = gdrcd_query("SELECT * FROM araldo_letto WHERE nome = '" . $_SESSION['login'] . "' AND thread_id = " . gdrcd_filter('num',
                            $_REQUEST['what']));
                    if ($check_letto['id'] <= 0)
                    {
                        gdrcd_query("INSERT INTO araldo_letto (nome, araldo_id, thread_id) VALUES ('" . $_SESSION['login'] . "', " . gdrcd_filter('num',
                                $_REQUEST['where']) . ", " . gdrcd_filter('num', $_REQUEST['what']) . ")");
                    }
                    ?>
                    <div class="panels_box">
                        <table>
                                ...
                                <td class="forum_main_post_message">
                                    ...
                                    <div class="forum_post_modify">
                                        ...
                                        if ($chiuso == 0 || $_SESSION['permessi'] >= MODERATOR)
                                        {
                                            ...
                                            <a href="main.php?page=forum&op=composer&what=<?php echo gdrcd_filter('num',
                                                $_REQUEST['what']); ?>&where=<?php echo gdrcd_filter('num',
                                                $_REQUEST['where']); ?>&quote=<?php echo $row['id_messaggio']; ?>">[<?php echo gdrcd_filter('out',
                                                    $MESSAGE['interface']['forums']['link']['quote']); ?>]</a>
                                            <?php
                                        }
                                        ...
                                        {
                                            ?>
                                            <a href="main.php?page=forum&op=modifica&what=<?php echo $row['id_messaggio']; ?>">[<?php echo $MESSAGE['interface']['forums']['link']['edit']; ?>
                                                ]</a>
                                            <a href="main.php?page=forum&op=delete_conf&id_record=<?php echo $row['id_messaggio']; ?>&padre=<?php echo $row['id_messaggio_padre']; ?>">[<?php echo gdrcd_filter('out',
                                                    $MESSAGE['interface']['forums']['link']['delete']); ?>]</a>
                                            <?php
                                        }
                                        ?>
                                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="forum_other_post_message">
                                        ...
                                        <div class="forum_post_modify">
                                            ...
                                            {
                                                ?>
                                                <a href="main.php?page=forum&op=modifica&what=<?php echo $row['id_messaggio']; ?>">[<?php echo gdrcd_filter('out',
                                                        $MESSAGE['interface']['forums']['link']['edit']); ?>]</a>
                                                <a href="main.php?page=forum&op=delete_conf&id_record=<?php echo $row['id_messaggio']; ?>&padre=<?php echo $row['id_messaggio_padre']; ?>">[<?php echo gdrcd_filter('out',
                                                        $MESSAGE['interface']['forums']['link']['delete']); ?>]</a>
                                                <?php
                                            }
                ...
                if ($chiuso == 0 || $_SESSION['permessi'] >= MODERATOR)
                {
                    ?>
                    <a href="main.php?page=forum&op=composer&what=<?php echo gdrcd_filter('num',
                        $_REQUEST['what']); ?>&where=<?php echo gdrcd_filter('num', $_REQUEST['where']); ?>">
            ...
            else
            {
                echo '<div class="error">' . gdrcd_filter('out',
                        $MESSAGE['interface']['forums']['warning']['topic_not_exists']) . '</div>';
            }
            ?>
        ...
        if (isset($_REQUEST['op']) === false)
        {
            ...
            $result = gdrcd_query("SELECT id_araldo, nome, tipo, proprietari FROM araldo ORDER BY tipo, nome",
                'result');
                                ...
                                )
                                {
                                    ...
                                    <tr><!-- Intestazione tabella -->
                                        <td colspan="2">
                                            <div class="capitolo_elenco">
                                                <?php echo gdrcd_filter('out',
                                                    $PARAMETERS['names']['forum']['plur'] . ' ' . strtolower($MESSAGE['interface']['forums']['type'][$ultimotipo])); ?>
                                            </div>
                                        </td>
                                    </tr>
                                    <?php
                            ...
                            <tr><!-- Forum della categoria -->
                                ...
                                <td class="casella_elemento">
                                    <div class="elementi_elenco">
                                        ...
                                        <a href="main.php?page=forum&op=visit&what=<?php echo gdrcd_filter('out',
                                            $row['id_araldo']); ?>&name=<?php echo gdrcd_filter('out',
                                            $row['nome']); ?>">
        ...
        {
            //Permessi
            $row = gdrcd_query("SELECT tipo, proprietari FROM araldo WHERE id_araldo = " . gdrcd_filter('num',
                    $_REQUEST['what']) . "");
            ...
            } else
            {
                ...
                $result = gdrcd_query("SELECT MA.id_messaggio, MA.titolo, MA.autore, MA.data_messaggio, MA.importante, MA.chiuso, AL.id AS new_msg FROM messaggioaraldo AS MA LEFT JOIN araldo_letto AS AL ON MA.id_messaggio=AL.thread_id AND AL.nome='" . $_SESSION['login'] . "' WHERE MA.id_messaggio_padre = -1 AND MA.id_araldo = " . gdrcd_filter('num',
                        $_REQUEST['what']) . " ORDER BY MA.importante DESC, MA.data_messaggio DESC LIMIT " . $pagebegin . ", " . $PARAMETERS['settings']['posts_per_page'] . "",
                    'result');

                if (gdrcd_query($result, 'num_rows') == 0)
                {
                    ?>
                    <div class="warning"><?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['forums']['warning']['no_topic']); ?></div>
                    <?php
                } else
                {
                    ?>
                            ...
                            <table>
                                ...
                                <tr><!-- Intestazione tabella -->
                                    <td class="casella_titolo">
                                        <div class="capitolo_elenco">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['forums']['topic']['title']); ?>
                                        </div>
                                    </td>
                                    <td class="casella_titolo">
                                        <div class="capitolo_elenco">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['forums']['topic']['author']); ?>
                                        </div>
                                    </td>
                                    <td class="casella_titolo">
                                        <div class="capitolo_elenco">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['forums']['topic']['posts']); ?>
                                        </div>
                                    </td>
                                    <?php
                                    if ($_SESSION['permessi'] >= MODERATOR)
                                    ...
                                    <tr><!-- Topic -->
                                        <td class="casella_elemento">
                                            <div class="elementi_elenco"><!-- Titolo -->
                                                <a href="main.php?page=forum&op=read&what=<?php echo gdrcd_filter('out',
                                                    $row['id_messaggio']); ?>&where=<?php echo gdrcd_filter('num',
                                                    $_REQUEST['what']); ?>">
                                                ...
                                                ?>
                                                <div
                                                    class="forum_date_big"><?php echo gdrcd_format_date($row['data_messaggio']) . ' ' . gdrcd_format_time($row['data_messaggio']); ?></div>
                                        ...
                                        <td class="casella_elemento">
                                            <div class="elementi_elenco"><!-- Data -->
                                                <?php echo $postsnumber . ' ' . gdrcd_filter('out',
                                                        $MESSAGE['interface']['forums']['topic']['posts']); ?>
                                                <div class="forum_date_big"><?php if ($postsnumber > 0)
                                                    {
                                                        echo gdrcd_filter('out',
                                                                $MESSAGE['interface']['forums']['topic']['last_post']) . ':   ' . gdrcd_format_date($lastupdate) . ' ' . gdrcd_format_time($lastupdate);
                                                    } ?></div>
                                            </div>
                                        </td>
                                        <?php
                ...
                <div class="pager">
                    ...
                    if ($totaleresults > $PARAMETERS['settings']['posts_per_page'])
                    {
                        ...
                        for ($i = 0; $i <= floor($totaleresults / $PARAMETERS['settings']['posts_per_page']); $i++)
                        {
                            ...
                            {
                                ?>
                                <a href="main.php?page=forum&op=visit&what=<?php echo gdrcd_filter('num',
                                    $_REQUEST['what']) ?>&offset=<?php echo $i; ?>"><?php echo $i + 1; ?></a>
                                <?php
                            } else
                            {
                                echo ' ' . ($i + 1) . ' ';
                ...
                <!-- link crea nuovo -->
                <div class="link_back">
                    <a href="main.php?page=forum&op=composer&what=-1&where=<?php echo gdrcd_filter('num',
                        $_REQUEST['what']); ?>">
