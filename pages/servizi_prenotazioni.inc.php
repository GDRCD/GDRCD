            ...
            <div class="form_gioco">
                ...
                if ((gdrcd_query($result, 'num_rows') == 0) || ($PARAMETERS['mode']['privaterooms'] == 'OFF'))
                {
                    echo '<div class="warning">' . gdrcd_filter('out',
                            $MESSAGE['interface']['hotel']['no_room']) . '</div>';
                            ...
                            <select name="id">
                                ...
                                { ?>
                                    <option value="" disabled>
                                        <?php echo gdrcd_filter('out',
                                                $row['luogo'] . ', ' . $row['nome']) . ' (' . $row['proprietario'] . ', ' . gdrcd_format_time($row['scadenza']) . ') '; ?>
                                    </option>
                                ...
                                { ?>
                                    <option value="<?php echo $row['id'] . '-' . $row['costo']; ?>">
                                        <?php echo gdrcd_filter('out',
                                                $row['luogo'] . ', ' . $row['nome']) . ' (' . $row['costo'] . ' ' . strtolower($PARAMETERS['names']['currency']['plur']) . ' ' . $MESSAGE['interface']['hotel']['per_hour'] . ') '; ?>
                            ...
                            <select name="ore">
                                ...
                                { ?>
                                    <option value="<?php echo $i; ?>">
                                        <?php echo $i . ' ' . gdrcd_filter('out',
                                                $MESSAGE['interface']['hotel']['hours']); ?>
                                    </option>
                                <?php }//while
                                ?>
                            </select>
        ...
        if (gdrcd_filter('get', $_POST['op']) == 'book')
        {
            ...
            } else
            {
                ...
                if ($soldi['soldi'] >= ($ore * $id[1]))
                {
                    ...
                    gdrcd_query("UPDATE mappa SET proprietario = '" . $_SESSION['login'] . "', invitati='', ora_prenotazione=NOW(), scadenza=DATE_ADD(NOW(), INTERVAL " . $_POST['ore'] . " HOUR) WHERE id = " . gdrcd_filter('num',
                            $id[0]) . " and scadenza < NOW() LIMIT 1");

                    gdrcd_query("UPDATE personaggio SET soldi = soldi - " . gdrcd_filter('num',
                            $ore * $id[1]) . " WHERE nome = '" . $_SESSION['login'] . "' LIMIT 1");
                ...
                } else
                {
                    echo '<div class="error">' . gdrcd_filter('out',
                            $MESSAGE['interface']['hotel']['no_bucks']) . '</div>';
                }
            }//else
            ?>

            <div class="panels_link">
                <a href="main.php?page=servizi_prenotazioni"><?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['hotel']['back']); ?></a>
            </div>
