            ...
            <?php if ($_SESSION['permessi'] >=Functions::get_constant('QUEST_VIEW') || $_REQUEST['pg']==$_SESSION['login']) { ?>
                <form action="main.php?page=scheda_px&pg=<?php echo gdrcd_filter('get',$_REQUEST['pg']); ?>" method="post">
                    <div class="form_submit">
                        ...
                        <input type="hidden" value="PX" name="op" />
                    </div>
                </form>
                <form action="main.php?page=scheda_px&pg=<?php echo gdrcd_filter('get',$_REQUEST['pg']); ?>" method="post">
                    <div class="form_submit">
                        <input type="submit" value="Resoconti quest" />
                        <input type="hidden" value="quest" name="op" />
                    </div>
                </form>
            <?php
            }
            if ($_POST['op'] == 'PX' || isset($_POST['op']) === FALSE) {
                ...
                <div class="elenco_record_gioco">
                    <table>
                        ...
                        while ($record=gdrcd_query($result, 'fetch')){
                            $quest_orig = "SELECT * FROM quest WHERE id= ".$record['id_quest']." ";
                            $resq=gdrcd_query($quest_orig, 'result');
                            $resf=gdrcd_query($resq, 'fetch');
                            ?>
                            <tr>
                                <!-- Oggetto, immagine, quantità -->
                                <td class="casella_elemento">
                                    <div class="elementi_elenco"><?php echo gdrcd_filter('out',$record['nome']); ?></div>
                                </td>
                                <td class="casella_elemento">
                                ...
                                <td class="casella_elemento">
                                    <div class="elementi_elenco">
                                        <?php echo gdrcd_filter('out',$resf['descrizione']); ?>
                                    </div>
                                </td>
                                <td class="casella_elemento">
                                    <div class="elementi_elenco">
                                        <?php echo gdrcd_filter('out',$record['px_assegnati']); ?>
                                    </div>
                                </td>
                                <td class="casella_elemento">
                                    <div class="elementi_elenco">
                                        <?php echo gdrcd_filter('out',$record['commento']); ?>
                                    </div>
                                </td>
                            </tr>
                            <?php
