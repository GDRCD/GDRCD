...
<div class="pagina_servizi_adm_gilde">
    ...
    <div class="page_title">
        <h2><?php echo gdrcd_filter('out',
                $MESSAGE['interface']['adm_guilds']['page_name'] . ' ' . strtolower($PARAMETERS['names']['guild_name']['plur'])); ?></h2>
    </div>
    ...
    <div class="page_body">
        ...
        { ?>
                        ...
                        <div class="form_element">
                            <select name="ruolo">
                                ...
                                while ($row = gdrcd_query($members_result, 'fetch'))
                                {
                                    ...
                                    ?>
                                    <option
                                        value="<?php echo $row['personaggio'] . "-" . $row['id_ruolo'] . "-" . $row['nome_ruolo']; ?>">
            ...
            <div class="link_back">
                <a href="main.php?page=servizi_adm_gilde"><?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['adm_guids']['back']); ?></a>
            </div>
        ...
        if (gdrcd_filter('get', $_POST['op']) == 'hire')
        {
            ...
            $jobs = gdrcd_query("SELECT COUNT(*) FROM clgpersonaggioruolo WHERE personaggio = '" . gdrcd_filter('in',
                    $_POST['nome']) . "'");
            ...
            if ($jobs['COUNT(*)'] >= $PARAMETERS['settings']['guilds_limit'])
            {
                echo '<div class="warning">' . gdrcd_filter('out',
                        $_POST['nome'] . ' ' . $MESSAGE['interface']['adm_guilds']['cannot_hire']) . '</div>';
            } else
            {
                /*Opero l'affiliazione*/
                $subject = explode('-', gdrcd_filter('in', $_POST['ruolo']));
                gdrcd_query("INSERT INTO clgpersonaggioruolo  (personaggio, id_ruolo, scadenza) VALUES ('" . gdrcd_filter('in',
                        $_POST['nome']) . "', " . $subject[0] . ", NOW())");

                /*Confermo l'operazione*/
                echo '<div class="warning">' . gdrcd_filter('out',
                        $MESSAGE['interface']['adm_guilds']['ok_hire']) . '</div>';
                ...
                gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento ,descrizione_evento) VALUES ('" . gdrcd_filter('in',
                        $_POST['nome']) . "', '" . $_SESSION['login'] . "', NOW(), " . NUOVOLAVORO . ", '" . gdrcd_filter('out',
                        $subject[1]) . "')");

                /*Avviso l'utente*/
                if ($_SESSION['login'] != $_POST['nome'])
                {
                    gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . gdrcd_filter('in',
                            $_POST['nome']) . "', NOW(), '" . gdrcd_filter('in',
            ...
            <div class="panels_link">
                <a href="main.php?page=servizi_adm_gilde"><?php echo gdrcd_filter('out',
                        $MESSAGE['interface']['adm_guilds']['back']); ?></a>
            </div>
        ...
        {
            $subject = explode('-', gdrcd_filter('in', $_POST['ruolo']));
            gdrcd_query("DELETE FROM clgpersonaggioruolo WHERE personaggio='" . $subject[0] . "' AND id_ruolo = " . gdrcd_filter('num',
                    $subject[1]) . " LIMIT 1");

            /*Confermo l'operazione*/
            echo '<div class="warning">' . gdrcd_filter('out',
                    $MESSAGE['interface']['adm_guilds']['ok_fire']) . '</div>';
            /*Registro l'operazione*/
            gdrcd_query("INSERT INTO log (nome_interessato, autore, data_evento, codice_evento ,descrizione_evento) VALUES ('" . $subject[0] . "', '" . $_SESSION['login'] . "', NOW(), " . DIMISSIONE . ", '" . gdrcd_filter('out',
                    $subject[2]) . "')");

            /*Avviso l'utente*/
            if ($_SESSION['login'] != $subject[0])
            {
                gdrcd_query("INSERT INTO messaggi (mittente, destinatario, spedito, testo) VALUES ('" . $_SESSION['login'] . "', '" . $subject[0] . "', NOW(), '" . gdrcd_filter('in',
