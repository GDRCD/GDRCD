    ...
    } else
    {
        ...
        <div class="page_title">
            <h2><?php echo gdrcd_filter('out',
                    $MESSAGE['interface']['administration']['maintenance']['page_name']); ?></h2>
        </div>
        ...
        <div class="page_body">
            ...
            if ($_POST['op'] == 'old_log')
            {
                ...
                )
                {
                    ...
                    gdrcd_query("DELETE FROM log WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > data_evento");
                ...
                <div class="link_back">
                    <a href="main.php?page=gestione_manutenzione">
                        <?php echo gdrcd_filter('out',
                            $MESSAGE['interface']['administration']['maintenance']['link']['back']); ?>
                    </a>
                </div>
                <?php
            }
            ...
            if (gdrcd_filter('get', $_POST['op']) == 'old_chat')
            {
                ...
                {
                    /*Eseguo l'aggiornamento*/
                    gdrcd_query("DELETE FROM chat WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora");
            ...
            if (gdrcd_filter('get', $_POST['op']) == 'old_messages')
            {
                ...
                {
                    /*Eseguo l'aggiornamento*/
                    gdrcd_query("DELETE FROM messaggi WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > spedito");
                    gdrcd_query("OPTIMIZE TABLE messaggi");
                    gdrcd_query("DELETE FROM backmessaggi WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > spedito");
            ...
            if (gdrcd_filter('get', $_POST['op']) == 'missing')
            {
                ...
                {
                    /*Eseguo l'aggiornamento*/
                    gdrcd_query("DELETE FROM clgpersonaggiooggetto WHERE nome IN (SELECT nome FROM personaggio WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora_entrata)");
                    gdrcd_query("OPTIMIZE TABLE clgpersonaggiooggetto");

                    gdrcd_query("DELETE FROM clgpersonaggioabilita WHERE nome IN (SELECT nome FROM personaggio WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora_entrata)");
                    gdrcd_query("OPTIMIZE TABLE clgpersonaggioabilita");

                    gdrcd_query("DELETE FROM clgpersonaggiomostrine WHERE nome IN (SELECT nome FROM personaggio WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora_entrata)");
                    gdrcd_query("OPTIMIZE TABLE clgpersonaggiomostrine");

                    gdrcd_query("DELETE FROM clgpersonaggioruolo WHERE personaggio IN (SELECT nome AS personaggio FROM personaggio WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora_entrata)");
                    gdrcd_query("OPTIMIZE TABLE clgpersonaggioruolo");

                    gdrcd_query("DELETE FROM personaggio WHERE DATE_SUB(NOW(), INTERVAL " . gdrcd_filter('num',
                            $_POST['mesi']) . " MONTH) > ora_entrata");
                    gdrcd_query("OPTIMIZE TABLE personaggio");
                    ?>
                    <!-- Conferma -->
            ...
            if (isset($_POST['op']) === false)
            {
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['old_log']); ?>
                        </div>
                        <div class='form_field'>
                            <select name="mesi" ?>
                                <?php for ($i = 0; $i <= 12; $i++)
                                { ?>
                                    <option value="<?php echo $i; ?>"><?php echo $i . ' ' . gdrcd_filter('out',
                                        $MESSAGE['interface']['administration']['maintenance']['months']); ?></option><?php } ?>
                            </select>
                        </div>
                        <!-- bottoni -->
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['old_chat']); ?>
                        </div>
                        <div class='form_field'>
                            <select name="mesi" ?>
                                <?php for ($i = 0; $i <= 12; $i++)
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['old_messages']); ?>
                        </div>
                        <div class='form_field'>
                            <select name="mesi" ?>
                                <?php for ($i = 0; $i <= 12; $i++)
                        ...
                        <div class='form_info'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['old_messages_info']); ?>
                        </div>
                        <!-- bottoni -->
                        <div class='form_submit'>
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['deleted']); ?>
                        </div>
                        <div class='form_info'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['deleted_info']); ?>
                        </div>
                        <!-- bottoni -->
                        <div class='form_submit'>
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['missing']); ?>
                        </div>
                        <div class='form_field'>
                            <select name="mesi" ?>
                                <?php
                                for ($i = 1; $i <= 12; $i++)
                                {
                                    ?>
                                    <option value="<?php echo $i; ?>"><?php echo $i . ' ' . gdrcd_filter('out',
                                                $MESSAGE['interface']['administration']['maintenance']['months']); ?></option>
                                    <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class='form_info'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['missing_info']); ?>
                        </div>
                        <!-- bottoni -->
                        <div class='form_submit'>
                ...
                <div class="panels_box">
                    <form action="main.php?page=gestione_manutenzione"
                        ...
                        <div class='form_label'>
                            <?php echo gdrcd_filter('out',
                                $MESSAGE['interface']['administration']['maintenance']['blacklisted']); ?>
                        </div>
                        <div class='form_info'>
                            <?php echo gdrcd_filter('out',
