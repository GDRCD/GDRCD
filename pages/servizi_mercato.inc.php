<div class="pagina_servizi_mercato">
    ...
    <div class="page_body">
        ...
        if ($_POST['op'] == 'buy')
        {
            ...
            $costo = gdrcd_query("SELECT cariche, costo FROM oggetto WHERE id_oggetto = " . gdrcd_filter('num',
                    $_POST['id_oggetto']) . "");

            if ($money >= $costo['costo'])
            {
                /*Controllo se possiede gia' oggetti analoghi*/
                $query = "SELECT id_oggetto FROM clgpersonaggiooggetto WHERE id_oggetto = " . gdrcd_filter('num',
                        $_POST['id_oggetto']) . " AND nome = '" . $_SESSION['login'] . "'";
                ...
                {
                    $query = "INSERT INTO clgpersonaggiooggetto (nome, id_oggetto, cariche, numero, posizione) VALUES ('" . $_SESSION['login'] . "'," . gdrcd_filter('num',
                            $_POST['id_oggetto']) . ", " . $costo['cariche'] . ", 1, 0)";
                }
                /*Eseguo l'acquisto*/
                gdrcd_query($query);
                /*Esigo il quattrino*/
                ...
                {
                    $query = "UPDATE mercato SET numero = numero - 1 WHERE id_oggetto = '" . gdrcd_filter('num',
                            $_POST['id_oggetto']) . "' LIMIT 1";
                } else
                {
                    $query = "DELETE FROM mercato WHERE id_oggetto = '" . gdrcd_filter('num',
                            $_POST['id_oggetto']) . "' LIMIT 1";
                }
                gdrcd_query($query);
        ...
        if (gdrcd_filter('get', $_POST['op']) == 'sell')
        {
            ...
            $query = "SELECT clgpersonaggiooggetto.numero, oggetto.costo FROM clgpersonaggiooggetto LEFT JOIN oggetto ON clgpersonaggiooggetto.id_oggetto = oggetto.id_oggetto WHERE clgpersonaggiooggetto.id_oggetto = " . gdrcd_filter('num',
                    $_POST['id_oggetto']) . " AND clgpersonaggiooggetto.nome = '" . $_SESSION['login'] . "'";
            #echo $query;
            $result = gdrcd_query($query, 'result');
            ...
            if (gdrcd_query($result, 'num_rows') > 0)
            {
                ...
                <?php
                gdrcd_query("UPDATE mercato SET numero = numero + 1 WHERE id_oggetto = " . gdrcd_filter('num',
                        $_POST['id_oggetto']) . " LIMIT 1");

                gdrcd_query("UPDATE personaggio SET soldi = soldi + " . gdrcd_filter('num',
                        $costo_vendita) . " WHERE nome = '" . $_SESSION['login'] . "' LIMIT 1");
                            ...
                            <tr>
                                <td class="casella_elemento">
                                    <div class="elementi_elenco">
                                        <a href="main.php?page=servizi_mercato&op=visit&what=<?php echo gdrcd_filter('out',
                                            $row['cod_tipo']); ?>"><?php echo gdrcd_filter('out',
                                                $row['descrizione']); ?></a>
                                    </div>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td class="casella_titolo">
                                <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['market']['item']); ?></div>
                            </td>
                            <td class="casella_titolo">
                                <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['market']['bonus']); ?></div>
                            </td>
                            <td class="casella_titolo">
                                <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['market']['more']); ?></div>
                            </td>
                            <td class="casella_titolo">
                                <div class="titoli_elenco"><?php echo gdrcd_filter('out',
                                        $MESSAGE['interface']['market']['price']); ?></div>
                            </td>
                        </tr>
                        <!-- Record -->
                        <?php while ($row = gdrcd_query($result, 'fetch'))
                            ...
                            <tr>
                                <td class="casella_elemento_img">
                                    ...
                                    <div class="inventario_img">
                                        <img
                                            src="themes/<?php echo $PARAMETERS['themes']['current_theme']; ?>/imgs/items/<?php echo $row['urlimg']; ?>"/>
                                    </div>
                                    <div
                                        class="inventario_quantita"><?php echo $MESSAGE['interface']['market']['stock'] . ': ' . $row['numero']; ?></div>
                                </td>
                                <td class="casella_elemento">
                                    <div class="inventario_riga_proprieta">
                                        <div style="float: left; clear: left;">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['market']['attack'] . ': '); ?>
                                        </div>
                                    ...
                                    <div class="inventario_riga_proprieta">
                                        <div style="float: left; clear: left;">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['market']['defence'] . ': '); ?>
                                        </div>
                                        <div style="float: right; clear: right;">
                                            <?php echo $row['difesa']; ?>
                                        </div>
                                    ...
                                    <div class="inventario_riga_proprieta">
                                        <div style="float: left; clear: left;">
                                            <?php echo gdrcd_filter('out',
                                                $MESSAGE['interface']['market']['item_charges'] . ': '); ?>
                                        </div>
                                        <div style="float: right; clear: right;">
                                    ...
                                    <div class="controlli_elenco">
                                        <div class="form_gioco">
                                            <form action="main.php?page=servizi_mercato" method="post">
                                                <input type="hidden" name="id_oggetto"
                                                       value="<?php echo $row['id_oggetto'] ?>"/>
                                                <input type="hidden" name="costo" value="<?php echo $row['costo'] ?>"/>
                                                <input type="hidden" name="cariche"
                                                       value="<?php echo $row['cariche'] ?>"/>
                                        ...
                                        <div class="form_gioco">
                                            <form action="main.php?page=servizi_mercato" method="post">
                                                ...
                                                <input type="hidden" name="costo"
                                                       value="<?php echo ($row['costo'] / 100) * (100 - $PARAMETERS['settings']['resell_price']) ?>"/>
