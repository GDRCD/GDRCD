...
if ((isset($_POST['op']) === true) && (($_POST['op'] == gdrcd_filter('out',
                $MESSAGE['interface']['maps']['leave'])) || ($_POST['op'] == gdrcd_filter('out',
                $MESSAGE['interface']['maps']['arrive'])))
)
{
    /*Aggiorno la sua posizione*/
    gdrcd_query("UPDATE mappa_click SET posizione = " . gdrcd_filter('num',
            $_POST['destination']) . " WHERE id_click = " . gdrcd_filter('num', $_REQUEST['map_id']) . " LIMIT 1");
}
...
} else
{
    ...
    while ($row = gdrcd_query($result, 'fetch'))
    {
        ...
        if ($redirect_pc == 1)
        {
            gdrcd_query("UPDATE personaggio SET ultima_mappa=" . gdrcd_filter('get',
                    $row['id_click']) . " WHERE nome = '" . gdrcd_filter('in', $_SESSION['login']) . "'");
        }
    ...
    if ($vicinato != INVIAGGIO)
    {
        ...
        <div class="mappaclick_more">
            ...
            $result = gdrcd_query("SELECT id_click, nome FROM mappa_click WHERE posizione = " . $vicinato . " AND id_click <> " . $self . " ORDER BY nome",
                'result');
            ...
            { ?>
                <form class="form_gioco"
                    ...
                    <?php } else
                    {
                        ...
                        $result = gdrcd_query("SELECT posizione, nome FROM mappa_click WHERE posizione <> -1 AND id_click <> " . $_SESSION['mappa'] . " ORDER BY nome",
                            'result');
