{foreach $body_rows as $row}
    <div class='tr'>
        ...
        <div class='td commands'>
            <a href='/main.php?page={$path}&op=view&id={$row.id}&id_pg={$row.id_pg}&pg={$row.pg}' title='Visualizza'><i class='fas fa-file'></i></a>
            {if $row.contatti_view_permission}
                <a href='/main.php?page={$path}&op=edit&id={$row.id}&id_pg={$row.id_pg}&pg={$row.pg}' title='Modifica'><i class='fas fa-edit'></i></a>
